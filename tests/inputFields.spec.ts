...
test.describe('Input fields', () => {
    ...
    test('Test Case 1: Update pet type', async ({page}) => {
        ...
        const petTableRows = page.locator('tbody tr')
        await petTableRows.getByRole('button',{name: "Edit"}).first().click()
        ...
        await expect(petTableRows.locator('[id="0"]')).toHaveValue('rabbit')
        ...
        await expect(petTableRows.locator('[id="0"]')).toHaveValue('cat')
    })
