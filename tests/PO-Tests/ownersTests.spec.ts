...
test.describe('Date Selector Tests', () => {
    ...
    test('TC_6 Verify adding and deletion of the Pet with validation of the Pet details display', async ({ page }) => {
        ...
        const petName = await pm.onAddNewPetPage().fillInPetNameToTheNameFieldAndAssertNameIsValidAndCheckIconIsAppeared("Tom")
        ...
        const petType = await pm.onAddNewPetPage().selectPetTypeThenExtractValueAndSaveTheNewPetDetails("dog")
        await pm.onOwnerInformationPage().validatePetDetailsForPet(petName, petBirthDate, petType)
        await pm.onOwnerInformationPage().deleteTargetedPetAndValidateTheWasRemovedFromTheList(petType)
    })
    ...
    test('TC_7 Verify owner details are displayed on the Owner information page', async ({page}) => {
        ...
        const ownerDetailsForAssertion = await pm.onOwnerInformationPage().extractOwnerDetails()
    ...
    test('TC_8 Verify owner and Pet details are displayed on the New visit page', async ({page}) => {
        ...
        await pm.onOwnersPage().clickOnTheOwnerFullNameLink("Jean Coleman")
        const ownerDetailsForAssertion = await pm.onOwnerInformationPage().extractOwnerDetails()
        const targetedPetDetails = await pm.onOwnerInformationPage().getTargetedPetDetails("Samantha")
    ...
    test('TC_10 Verify editing Pet and validate Pet type change', async ({page}) => {
        ...
        const ownerDetails = await pm.onOwnerInformationPage().extractOwnerDetails()
        const petDetails = await pm.onOwnerInformationPage().getTargetedPetDetails("Leo")
        ...
        const petType = await pm.onEditPetPage().selectPetTypeAndClickUpdatePetButton("dog")
        await pm.onOwnerInformationPage().validatePetDetailsForPet(petDetails[0], petDetails[1], petType)
    })
})